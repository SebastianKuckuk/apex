...
inline int realMain(int argc, char *argv[]) {
    ...
    {
        Kokkos::View<tpe *> dest("dest", nx * std::max(strideRead, strideWrite));
        Kokkos::View<tpe *> src("src", nx * std::max(strideRead, strideWrite));

        auto h_dest = Kokkos::create_mirror_view(dest);
        auto h_src = Kokkos::create_mirror_view(src);

        // init
        initStreamStrided(h_dest.data(), h_src.data(), nx, strideRead, strideWrite);
        ...
        // check solution
        checkSolutionStreamStrided(h_dest.data(), h_src.data(), nx, nIt + nItWarmUp, strideRead, strideWrite);
    }
    Kokkos::finalize();
